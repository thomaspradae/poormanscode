...
from pmc.versioning import executor_adapter_version
...
def test_provider_pool_selects_lanes_and_quarantines_auth_failure(
    ...
    db.reconcile_provider_credential(
        first["reservation_id"], status_code=401, actual_tokens=0, headers={}
    )
...
def test_provider_pool_skips_lane_with_insufficient_observed_tpm(
    tmp_path: Path, monkeypatch
):
    db = Database(tmp_path / "pmc.db")
    db.create_job(Job("PMC-X", tmp_path, "task"))
    ...
    selected = db.reserve_provider_credential("pool", "PMC-X", 1, 500)
    assert selected["credential_id"] == "pool-2"
    db.reconcile_provider_credential(
        selected["reservation_id"],
        status_code=None,
        actual_tokens=100,
        headers={
            "x-ratelimit-remaining-tokens-minute": "40",
        },
    )
    with pytest.raises(RuntimeError, match="no available credential"):
        db.reserve_provider_credential("pool", "PMC-X", 1, 500)
    delay = db.provider_next_available_seconds("pool")
...
def test_scheduler_conformance_gate(tmp_path: Path):
    ...
    assert row["adapter_version"] == executor_adapter_version("bash")
...
def test_model_request_budget_is_a_hard_ceiling(tmp_path: Path):
    ...
    accounting = ModelRequestAccounting(
        db, job_id="PMC-X", attempt_id=attempt, candidate=candidate, budget=budget
    )
