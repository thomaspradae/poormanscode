...
def test_guarded_sandbox_enforces_file_size_limit(tmp_path: Path):
    result = build_sandbox("guarded").run(
        tmp_path, "python -c \"open('large','wb').write(b'x'*200000)\"",
        env=scrubbed_environment(), network=False,
        limits=SandboxLimits(wall_seconds=5, cpu_seconds=5, file_bytes=1024),
    )
    assert result.returncode != 0


@pytest.mark.skipif(subprocess.run(["id", "pmc-worker"], capture_output=True, check=False).returncode != 0,
...
def test_restricted_worker_does_not_receive_secrets(tmp_path: Path, monkeypatch):
    ...
    result = build_sandbox("restricted-user").run(
        tmp_path, "test -z \"${PMC_TEST_API_KEY:-}\" && touch worker-proof",
        env=scrubbed_environment(), network=False, limits=SandboxLimits(wall_seconds=5),
    )
